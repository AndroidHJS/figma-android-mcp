...
export const visualsExtractor: ExtractorFn = (node, result, context) => {
  ...
  if (hasValue("fills", node) && Array.isArray(node.fills) && node.fills.length) {
    ...
    // If any fill is an image, flag this node for download so the AI knows
    // it needs a PNG render rather than trying to recreate it with code.
    const hasImageFill = fills.some((f) => f && typeof f === "object" && "type" in f && f.type === "IMAGE");
    if (hasImageFill && !context.traversalState.imageAssets.some((a) => a.nodeId === node.id)) {
  ...
  if (
    ...
    const defs = simplifyPropertyDefinitions(
      node.componentPropertyDefinitions as Record<
        string,
        { type: string; defaultValue: boolean | string; variantOptions?: string[]; preferredValues?: { type: string; key: string }[] }
      >,
    );
 ...
 * it to IMAGE-PNG. BOOLEAN_OPERATION is included because it's both a collapsible container AND
 * SVG-eligible as a child (boolean ops always produce vector output).
 */
export const RASTER_ELIGIBLE_TYPES = new Set([
  "IMAGE-PNG", // VECTOR nodes are converted to IMAGE-PNG, or containers that were collapsed
  "BOOLEAN_OPERATION",
  "STAR",
  "LINE",
  "ELLIPSE",
...
/**
 * afterChildren callback that collapses raster-only containers to IMAGE-PNG.
 *
 * If a FRAME, GROUP, INSTANCE, or BOOLEAN_OPERATION contains only raster-eligible children, the parent
 * is marked as IMAGE-PNG and children are omitted, reducing payload size.
 *
 ...
 */
export function collapseRasterContainers(
  node: FigmaDocumentNode,
  result: SimplifiedNode,
  children: SimplifiedNode[],
): SimplifiedNode[] {
  const allChildrenAreRasterEligible = children.every((child) => RASTER_ELIGIBLE_TYPES.has(child.type));
  ...
  if (
    ...
    allChildrenAreRasterEligible &&
    !hasImageFillInChildren(node)
  ) {
    // Collapse to IMAGE-PNG and omit children
