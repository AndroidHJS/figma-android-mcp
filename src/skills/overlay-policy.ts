...
- **toast**：事件触发显示、自动消失。<!--compose-->Compose 用 \`SnackbarHost\` 或状态门控的 \`Box\` + \`LaunchedEffect\` 延时关闭。<!--/compose--><!--views-->传统 View 用 \`Toast\`/\`Snackbar\` API。<!--/views-->样式（背景色、圆角、图标、文案）照常取自节点设计数据，但**禁止**写进静态布局树。
- **dialog**：按 \`presentation\` 实现，显隐由状态控制，遮罩色用 \`scrim\` 值。<!--compose-->\`bottomSheet\` → \`ModalBottomSheet\`；\`centerDialog\` → \`Dialog\`。<!--/compose--><!--views-->\`bottomSheet\` → \`BottomSheetDialog\`；\`centerDialog\` → \`Dialog\` 主题。<!--/views-->弹窗内容带 \`straddle\`/\`anchorOverhang\` 标注时，弹窗容器必须透明化——见越界元素规则。
