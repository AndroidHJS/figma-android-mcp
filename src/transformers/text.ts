...
export function extractTextStyle(n: FigmaDocumentNode) {
  if (hasTextStyle(n)) {
    ...
    const textStyle: SimplifiedTextStyle = {
      ...
      fontSize: style.fontSize ? spString(style.fontSize) : undefined,
...
function classifyRun(
  ...
  for (const [key, rawValue] of Object.entries(delta)) {
    ...
    switch (key) {
      ...
      case "fontSize": {
        refDelta.fontSize = spString(value as number);
        hasRefProps = true;
        break;
      }
      case "letterSpacing": {
      ...
      case "paragraphSpacing": {
        if (typeof value === "number" && value > 0) {
          refDelta.paragraphSpacing = dpString(value);
          hasRefProps = true;
        }
        break;
      }
      case "paragraphIndent": {
        if (typeof value === "number" && value > 0) {
          refDelta.paragraphIndent = dpString(value);
          hasRefProps = true;
        }
        break;
      }
      case "listSpacing": {
        if (typeof value === "number" && value > 0) {
          refDelta.listSpacing = dpString(value);
          hasRefProps = true;
        }
        break;
      }
